...
install_now(){
	# default value
	local TITLE="网心云（KoolCenter版）"
	local DESCR="（KoolCenter版）"
	local PLVER=$(cat ${DIR}/version)

	# stop first
	local ENABLE=$(dbus get ${module}_enable)
	if [ "${ENABLE}" == "1" -o -n "$(pidof wangxinyun)" ];then
		echo_date "安装前先关闭${TITLE}插件，以保证更新成功！"
		killall	wangxinyun > /dev/null 2>&1
	fi

	# remove some file first
	find /koolshare/init.d -name "*wangxinyun.sh" | xargs rm -rf >/dev/null 2>&1

	# isntall file
	echo_date "安装插件相关文件..."
	cd /tmp
	rm -rf /koolshare/bin/wangxinyun
	cp -rf /tmp/${module}/bin/wangxinyun /koolshare/bin/
	...
	chmod 755 /koolshare/bin/wangxinyun/wxinit >/dev/null 2>&1 
